...
def install_node(node_version='latest', *args, **kwargs):
    ...
    if cmd_avail('n') and not exists('{n_prefix}/n/versions/node/{version}'.format(n_prefix=n_prefix,
                                                                                   version=node_version)):
        run_cmd('n {version}'.format(version=node_version))
    else:
        run_cmd("curl -L https://git.io/n-install"
                " | N_PREFIX='{n_prefix}' bash -s -- -y '{version}'".format(n_prefix=n_prefix, version=node_version))
